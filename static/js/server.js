...
app.post('/api/book', async (req, res) => {
	if (!db) return res.status(500).send('Database not available. Please try again later.');
...
app.route('/signup')
	...
	.post(async (req, res) => {
		if (!db) return res.render('signup.html', { error: 'Database not available' });
...
app.route('/login')
	...
	.post(async (req, res) => {
		if (!db) return res.render('login.html', { error: 'Database not available' });
...
app.get('/dashboard', requireLogin, async (req, res) => {
	...
	if (!db) return res.render('error.html', { code: 500, message: 'Database not available' });
